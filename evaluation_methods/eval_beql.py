...
def beql_metric(
    ...
    """Метрика для `map_metric`: эквивалентны ли эталон и предсказание по BEqL."""
    return beql(
...
def report(y_true: list[int], y_pred: list[int]) -> None:
    ...
    print("\n" + "=" * 48)
    print("BEqL как предиктор человеческой метки `correct`")
    print("=" * 48)
    print(f"Примеров:               {n}")
    print(f"Доля эквивалентных (BEqL): {sum(y_pred) / n:.2%}")
    ...
    print("-" * 48)
    print("Матрица ошибок (строки — истина, столбцы — BEqL):")
...
# --------------------------------------------------------------------------- #
def main() -> None:
    parser = argparse.ArgumentParser(description="BEqL-оценка формализаций в Lean 4.")
    parser.add_argument(
        "--dataset",
        type=str,
        default=DEFAULT_DATASET,
        help="HuggingFace id или путь к локальному .csv/.jsonl",
    )
    parser.add_argument(
        "--num-processes",
        type=int,
    ...
    # 3. прогон BEqL по всем примерам
    ...
    print(f"Запускаем BEqL в {args.num_processes} процессов...")
    preds = map_metric(
        records,
        metric,
        config,
